...
| `--description` | `-d` | string | Plain text or ADF description. **For structured formatting (headings, bold, lists, code blocks), send ADF JSON via `--description-file`** — wiki markup like `h2.` / `*bold*` is stored as literal escaped text, not parsed. See "Writing Ticket Bodies (ADF)" in SKILL.md for the ADF JSON schema and examples. |
| `--description-file` | | string | Read description from file. For ADF, the file must contain valid ADF JSON (not wiki markup). |
...
| `--description` | `-d` | string | New description. **For structured formatting, send ADF JSON via `--description-file`** — wiki markup is not parsed. See "Writing Ticket Bodies (ADF)" in SKILL.md. |
