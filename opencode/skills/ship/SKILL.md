...
Commits staged changes, pushes the branch, opens a PR, and triggers an automated
opencode review.
...
- Body bullet points for non-obvious details (optional but preferred for large
  changes)
...
- **Never commit** `.env`, secrets, or credential files — warn the user if
  they're staged
...
- If the build/tests are known to exist (e.g. `go build ./...`, `npm test`), run
  them before committing and abort if they fail
